...
def solve() -> dict:
    ...
    topic_result = DimReductionEngine().reduce(
        matrix_df, algorithm="nmf", n_components=N_TOPICS, seed=42
    )
    topic_weights = np.asarray(topic_result.transformed)
    doc_topics = topic_weights.argmax(axis=1).astype(int)

    category_ids = np.array(
        [CATEGORIES.index(category) for category in frame["category"].to_list()]
    )

    return {
        "doc_topics": doc_topics.tolist(),
        "n_topics": N_TOPICS,
        "topic_purity": _purity(category_ids, doc_topics),
    }
